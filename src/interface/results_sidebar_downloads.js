    ...
    this.props.struct_job_names.forEach((name) =>{
      ...
      if(this.props.analyses.includes(name+'_job')){
        ...
        if(name === this.props.job_strings.merizo.varName){
          link_data = this.createDownloadLinks(count, [['.pdb2', this.props.job_strings.merizo.shortName+' Annotated PDB'], ['.dom_pdb', 'Save '+this.props.job_strings.merizo.shortName+' Domain PDB files'], ['.merizo', this.props.job_strings.merizo.shortName+' Boundaries']], this.props.job_strings.merizo.shortName+' DOWNLOADS');
          downloads_text.push(link_data[0]);
        }
